...
const CHECKLIST = [
  ...
  ['Попытка запрещённого действия', 'Пресет «Запрещённое действие»: нажмите «Стабилизировать» у закрытого портала — всплывёт причина, а в журнал попадёт запись «заблокировано».'],
  ...
  ['Ошибка', 'Повредите сохранение в localStorage (portal-lab:save:v1), обновите: баннер «Сохранение повреждено» и сброс к новой игре.'],
];
...
export function renderAbout() {
  ...
  return `
    ...
    <h2>Автотесты</h2>
    <p class="muted">${TEST_FILES.length} файлов, 62 теста. Запуск: <code>npm test</code>.</p>
    ${list(TEST_FILES)}
  `;
}
